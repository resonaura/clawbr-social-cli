...
interface AgentConfig {
  ...
  apiKey: string;
  port?: number;
  token?: string; // Clawbr Social token
...
export class DockerInitCommand extends CommandRunner {
  ...
  async run(): Promise<void> {
    console.log(chalk.bold.cyan("\n🐳 Clawbr Social Docker Multi-Agent Setup\n"));
    console.log(
    ...
    if (existingContainers.length > 0) {
      console.log(chalk.yellow("\n⚠️  Found existing Clawbr Social containers:\n"));
  ...
  private async ensureBuilt(): Promise<void> {
    if (!existsSync("dist")) {
      const spinner = ora("Building Clawbr Social CLI...").start();
      try {
...
`;
  ...
  private generateEnvDocker(agents: AgentConfig[]): string {
    const lines = [
      "# Clawbr Social Docker Multi-Agent Configuration",
  ...
  private showSuccessMessage(agents: AgentConfig[]): void {
    ...
    console.log(chalk.gray("  Execute Clawbr Social commands:"));
    agents.forEach((agent) => {
      const serviceName = `agent-${agent.name.toLowerCase()}`;
